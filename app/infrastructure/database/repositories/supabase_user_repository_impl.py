...
class SupabaseUserRepositoryImpl(SupabaseUserRepository):
    ...
    async def get_by_email(self, email: str) -> Optional[SupabaseUser]:
        ...
        try:
            ...
            response = await client.rpc("get_user_by_email", {"p_email": email}).execute()
    ...
    async def create_user(
        ...
        try:
            ...
            existing_profile = await admin_client.table("profiles").select("id").eq("email", email).execute()
            ...
            if not signup_response.user:
                raise ValueError("La respuesta de Supabase Auth no contiene un usuario.")
            ...
            if not user:
                raise ValueError(f"No se pudo obtener ni crear el perfil del usuario (ID: {user_id}).")

            return user
        except APIError as e:
            if "user already registered" in str(e.message).lower():
                raise DuplicateEntityError("Usuario", "email", email)
            raise ValueError(f"Error de API de Supabase al crear usuario: {e}")
        except Exception as e:
            raise ValueError(f"Error inesperado al crear usuario: {e}")
    ...
    async def update_user(
        ...
        try:
            ...
            user = await self.get_by_id(user_id)
            if not user:
                raise EntityNotFoundError(f"No se encontró el usuario con ID {user_id}")
            ...
            client = await self._supabase_client.client
            # Actualizar perfil
            await client.table("profiles").update(update_data).eq("id", str(user_id)).execute()
            ...
            if not updated_user:
                raise EntityNotFoundError(f"No se pudo obtener el usuario actualizado con ID {user_id}")
    ...
    async def authenticate(self, email: str, password: str) -> Optional[Tuple[SupabaseUser, str, str]]:
        """
        ...
        try:
            ...
            auth_response = await client.auth.sign_in_with_password({"email": email, "password": password})
